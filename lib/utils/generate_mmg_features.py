    ...
    for frame, poses in coords.items():
        ...
        for camera, pose in poses.items():
            ...
            for i, coords in enumerate(pose):
                ...
                features_output["node_{}".format(i+1)] = features.detach().numpy()
                print("node_{}".format(i+1))
            camera_output["camera_{}".format(camera[-1])] = features_output
            print("camera_{}".format(camera[-1]))
        output[frame_number_str] = camera_output
        print(frame_number_str)
        #save_args(output, save_path, 'node_features_test2', verbose=True) 
        dic.update({str(frame_number_str) : camera_output})
