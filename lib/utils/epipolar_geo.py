...
@tf.function
def convert_points_to_homogenous(points):
    '''
...
@tf.function
def symmetrical_epipolar_distnce(pts1, pts2, Fm, squared=True, eps=1e-8):
    if pts1.size(-1) == 2:
        pts1 = convert_points_to_homogenous(pts1)
    
    if pts2.size(-1) == 2:
        pts2 = convert_points_to_homogenous(pts2)
        
    F_t = tf.transpose(Fm, perm=[0, 2, 1])
    line1_in_2 = pts1 @ F_t
    line2_in_1 = pts2 @ Fm
    
    numerator = tf.pow(tf.math.reduce_sum((pts2 * line1_in_2), axis=2), 2)
    denominator_inv = 1.0 / tf.pow(tf.norm(line1_in_2[...,:2], axis=2), 2) + 1.0 / tf.pow(tf.norm(line2_in_1[...,:2], axis=2), 2)
    out = numerator * denominator_inv
    
    if squared:
        return out
    
    return tf.sqrt(out + eps)
    
if __name__ == "__main__":
    # test convert_points_to_homogenous
    points = tf.constant([[1.4, 2.3, 4.4], [-1.4, 5.3, 4.4]])
    print('points:')
    print(points)
    print('homogenous:')
    print(convert_points_to_homogenous(points))
    # test fundamental_from_projections
    ...
    F1=fundamental_from_projections(P1,P2)
